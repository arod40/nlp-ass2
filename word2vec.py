...
def naiveSoftmaxLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset):
    ...
    f = lambda w: centerWordVec * y_hat[w] - (
        centerWordVec if w == outsideWordIdx else 0
    )

    loss = -np.log(y_hat[outsideWordIdx])
    gradCenterVec = (
        -outsideVectors[outsideWordIdx] + np.transpose(outsideVectors) @ y_hat
    )
    gradOutsideVecs = np.stack([f(w) for w in range(outsideVectors.shape[0])])
    ### END YOUR CODE
...
def skipgram(
    ...
    for outsideWord in outsideWords:
        current_loss, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(
